...
#' @description
#' The \code{MI.test} function offers three approaches for testing multiple
...
#'
#' @details
#' The \code{MI.test} function calls \code{MI.stat} to calculate a modified
...
#' of testing methods are implemented: \itemize{ \item The nonparametric
...
#' the product of p-values is the combination for each resample.  \item The
...
#' Note that this test can be conservative at times.  \item The Bonferroni
...
#' individual Pearson statistics is large.  }
#'
#' Agresti and Liu (1999) discuss a marginal logit model approach that uses
...
#' from the \bold{geepack} package.  However, Bilder, Loughin, and Nettleton
...
#' marginal probabilities are small.
#'
#' @param data For \code{summary.data = FALSE}: a data frame containing the raw
#' data where rows correspond to the individual item response vectors, and
#' columns correspond to the items, W1, \ldots, WI and Y1, \ldots, YJ (in this
#' order).
#'
#' For \code{summary.data = TRUE}: a data frame containing 4 columns
#' generically named \code{W}, \code{Y}, \code{yj}, and \code{count} (one MRCV
#' case), or 5 columns named \code{W}, \code{Y}, \code{wi}, \code{yj}, and
#' \code{count} (two MRCV case).
...
#' for testing for MI: \code{"boot"} specifies a nonparametric bootstrap
#' procedure; \code{"rs2"} specifies a Rao-Scott second-order adjustment;
#' \code{"bon"} specifies a Bonferroni adjustment; \code{"all"} specifies all
#' three approaches.
#' @param B The desired number of bootstrap resamples.
#' @param B.max The maximum number of bootstrap resamples.  A resample is
#' thrown out if at least one of the J (one MRCV case) or IxJ (two MRCV case)
#' contingency tables does not have the correct dimension; \code{MI.test} uses
#' the first \code{B} valid resamples or all valid resamples if that number is
#' less than \code{B}.
#' @param summary.data A logical value indicating whether \code{data} is a
#' summary file containing the item response data instead of the raw data.
#' Only \code{type = "bon"} is available for \code{summary.data = TRUE}.
#' @param add.constant A positive constant to be added to all zero marginal
#' cell counts.
#' @param plot.hist A logical value indicating whether plots of the emprical
#' bootstrap sampling distributions should be provided.
#' @param print.status A logical value indicating whether bootstrap progress
#' updates should be provided.
#' @return --- \code{MI.test} returns a list containing at least
#' \code{general}, a list containing the following objects: \itemize{
...
#' argument. \item\code{summary.data}: The original value supplied to the
#' \code{summary.data} argument. \item\code{X.sq.S}: The modified Pearson
#' statistic; NA if at least one of the J (one MRCV case) or IxJ (two MRCV
#' case) contingency tables does not have the correct dimension.
#' \item\code{X.sq.S.ij}: A matrix containing the individual Pearson
#' statistics. }
#'
#' --- For \code{type = "boot"}, the primary list additionally includes
#' \code{boot}, a list containing the following objects: \itemize{
...
#' having at least one contingency table with incorrect dimension.
#' \item\code{p.value.boot}: The bootstrap p-value for the test of MMI or SPMI.
#' \item\code{p.combo.min.boot}: The bootstrap p-value for the minimum p-value
#' combination method. \item\code{p.combo.prod.boot}: The bootstrap p-value for
#' the product p-value combination method. \item\code{X.sq.S.star}: A numeric
#' vector containing the modified Pearson statistics calculated for each
#' resample. \item\code{X.sq.S.ij.star}: A matrix containing the individual
#' Pearson statistics calculated for each resample.
#' \item\code{p.combo.min.star}: A numeric vector containing the minimum
#' p-value calculated for each resample. \item\code{p.combo.prod.star}: A
#' numeric vector containing the product p-value calculated for each resample.
#' }
#'
#' --- For \code{type = "rs2"}, the primary list additionally includes
#' \code{rs2}, a list containing the following objects: \itemize{
#' \item\code{X.sq.S.rs2}: The Rao-Scott second-order adjusted Pearson
#' statistic. \item\code{df.rs2}: The degrees of freedom for testing the
#' second-order Rao-Scott adjusted Pearson statistic. \item\code{p.value.rs2}:
#' The p-value based on the Rao-Scott second-order adjustment. }
#'
#' --- For \code{type = "bon"}, the primary list additionally includes
#' \code{bon}, a list containing the following objects: \itemize{
#' \item\code{p.value.bon}: The Bonferroni adjusted p-value for the test of MMI
#' or SPMI. \item\code{X.sq.S.ij.p.bon}: A matrix containing the Bonferroni
#' adjusted p-values for the individual Pearson statistics. }
#'
#' --- For \code{type = "all"}, the list includes all of the above objects.
#'
#' --- \code{MI.stat} returns a list containing the following objects:
#' \itemize{ \item\code{X.sq.S}: Defined above. \item\code{X.sq.S.ij}: Defined
#' above. \item\code{valid.margins}: The number of contingency tables with
#' correct dimension. }
#' @seealso The \code{\link{genloglin}} function offers a generalized loglinear
...
MI.test <- function(data, I, J, type = "all", B = 1999, B.max = B,
  ...
  if (nvars == 2) {
    output <- SPMI.test(data, I, J, type,
      B = B, B.max = B.max,
      summary.data = summary.data, add.constant = add.constant, plot.hist = plot.hist,
      print.status = print.status
    )
